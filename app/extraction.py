...
def _safe_merge(fallback: dict[str, Any], llm: dict[str, Any]) -> dict[str, Any]:
    """Merge LLM output into fallback, skipping invalid values."""
    result = dict(fallback)
    for key, value in llm.items():
        if key not in result:
            continue
        # Skip None or empty values from LLM — keep fallback
        if value is None:
            continue
        # Ensure dates_worked is always a list
        if key == "dates_worked" and not isinstance(value, list):
            continue
        # Ensure raw_evidence is always a dict
        if key == "raw_evidence" and not isinstance(value, dict):
            continue
        # Ensure boolean fields are actually booleans
        bool_fields = {"overtime_paid", "break_deducted", "break_taken", "spread_of_hours", "premium_paid"}
        if key in bool_fields and not isinstance(value, (bool, type(None))):
            continue
        result[key] = value
    return result
...
def extract_document_facts(document_text: str, document_type: str, provider: str | None = None) -> ExtractedDocumentFacts:
    ...
    llm_data = client.extract_json(prompt, fallback=fallback)
    data = _safe_merge(fallback, llm_data)
